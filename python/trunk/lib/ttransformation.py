...
class TTransformation:
	def __init__(self, req):
		...
		self.repertoireTravail = sre.match("(/.*/)", req.canonical_filename).group(0)
	...
	def proceder(self):
		...
		if (os.access(self.fichierDemande + ".html", os.R_OK) == True):
			...
			self.resultat = self.retournerContenuFichierExistant(self.fichierDemande)
			...
			self.fichierXSLT = self.repertoireTravail + self. __extraireNomFeuilleStyle(self.fichierDemande + ".xml")
			...
			elif (os.access(self.fichierXSLT, os.R_OK) == True):
	...
	def __extraireNomFeuilleStyle(self, fichierXML):
		...
		noeud = pis[0]
		if (noeud != None):
			match = sre.match("href=['\"](.+?)['\"]", noeud.getContent())
